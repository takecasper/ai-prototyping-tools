...
## Data display — Accordion (inventory gap, native-via-vendor) [D]

Fifth data-display piece (2026-06-30), the second structural-divergence case after Tabs. **The
Acuity DS package ships NO Accordion component** — `dist/index.d.ts` exports Alert/Badge/Button/Card/
Checkbox/Dropdown/Heading/Icon/IconButton/Link/Modal/ProgressBar/Radio/Spinner/Tab/Tabs/Textarea/
TextInput and none is an accordion/disclosure/collapse. The modern app renders collapsible sections
with **react-bootstrap** instead: `<Accordion>` over `<Card>` (`syncJob.jsx:2-71` — `Accordion.Toggle`
as `Card.Header` + `Accordion.Collapse` + `useAccordionToggle`, `defaultActiveKey` single-open). So
Accordion is **native-via-vendor** — a real status quo built from a vendor lib, not a DS component (the
Table precedent).

**Tokens enshrined** (`--ds-accordion-*`, the card model): header label neutrals-darker `#333`, border
neutrals-light `#B8B8B8`, radius 8px (`ds-rounded-lg`), chevron acuity-blue `#364699` (rotates 90° open),
header fill the bootstrap card-header `rgba(0,0,0,.03)` flattened to `#f7f7f7` (the exact rendered tint
not separately `[R]`-captured — recorded gap, the §4j Card precedent). **Computed contrast**
(`contrast.mjs`, "one45 (2020s) Accordion"): label `#333`/white **12.63** (AAA); chevron `#364699`/white
**8.42** (AAA); the `#B8B8B8` card border is a thin decorative 1px rule (1.98, flagged 1.4.11). One
canonical API (`items` {header, body, defaultOpen?} + `single?`, single-open by default — the
`defaultActiveKey` model) absorbs both poles, but unlike List the **skin diverges structurally** (card
here vs legacy underline — the Tabs case). Because the **package** ships no Accordion, `acuity-canon`
resolves it through the bridge to a flagged interim — the mirror of `Table`/`List`. Pattern prototype
`src/prototypes/curriculum-groups/`. See `02` for the real legacy `.subheader-sticky.collapsible` and
reconciliation map §4m.

---

## Data display — Tree (inventory gap, native-via-vendor) [D]

Sixth data-display piece (2026-06-30), the third structural-divergence case after Tabs and Accordion.
**The Acuity DS package ships NO Tree component** — `dist/index.d.ts` has zero tree exports. The modern
app renders the curriculum hierarchy as an indented **tree-table** (`canvas_sync mappingTable.jsx` — a
recursive `getOne45Events(currics, indentation)` emitting `<td style={{paddingLeft: Nrem}}>` rows, an
`fa-angle-right`/`fa-angle-down` expand chevron, and `collapseEventIds` collapse state). So Tree is
**native-via-vendor / app-level** — a real status quo built from indented table rows, not a DS
component (the Table precedent).

**Tokens enshrined** (`--ds-tree-*`): node label neutrals-darker `#333`, chevron muted-fg `#5f5f5f`,
active-node accent acuity-blue `#364699`, rem-based per-level indent. **Computed contrast**
(`contrast.mjs`, "one45 (2020s) Tree"): label `#333`/white **12.63** (AAA); chevron `#5f5f5f` **6.39**
(AA); accent `#364699` **8.42** (AAA). One canonical API (recursive `nodes`) absorbs both poles, but
like Accordion the **skin diverges** (an indented tree-table here vs the legacy dynatree widget).
Because the **package** ships no Tree, `acuity-canon` resolves it through the bridge to a flagged
interim — the mirror of `Table`/`List`/`Accordion`. Pattern prototype `src/prototypes/program-structure/`.
See `02` for the real legacy dynatree skin and reconciliation map §4n.

---

## Data display — Timeline (one45-2020s-only) [D]

Seventh data-display piece (2026-06-30), the first **one45-2020s-only** piece. one45-2020s owns a real
status-history — the EPA requirements history (`StagesTrainingBundle/_history.scss`): a `.history-row`
flex layout of **date | description | status** columns, `#5F5F5F` bold headers, **1px `#f2f2f2`** row
dividers, 10px row padding, rendered server-side in the modern app. one45-legacy/webeval ships **no**
history or timeline at all, so Timeline is one45-2020s-only — the inverse of legacy-only Avatar.

**Tokens enshrined** (`--ds-timeline-*`): date `#5f5f5f` (the `.history` header colour), title
neutrals-darker `#333`, marker/rail acuity-blue `#364699` + neutrals-light `#B8B8B8`, `1px #f2f2f2`
divider, 10px row padding. **Computed contrast** (`contrast.mjs`, "one45 (2020s) Timeline"): date
`#5f5f5f`/white **6.39** (AA); title `#333`/white **12.63** (AAA); marker `#364699`/white **8.42**
(AAA). One canonical API (`entries` — `{date, title, description?, status?}`); the real `.history-row`
dated rows are generalised to a marker-rail timeline (a light affordance, recorded as such). Native in
one45-2020s + lowfi (sketch); `one45-legacy` (no history) + `acuity-canon` (package ships none) resolve
it through the bridge to a flagged interim. Pattern prototype `src/prototypes/assessment-history/`. See
reconciliation map §4o.
